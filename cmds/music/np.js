...
module.exports = {
    ...
    run: ({ message, client, embedResponse }) => {
        ...
        let embed = new MessageEmbed()
            ...
            .setDescription(`${`[${data.queue.current.isStream ? '<a:frog_rotate:720984862231887883>' : data.queue.current.fromPlaylist ? `<:mc_song:786660726914678834>` : '<a:songDJ:786662120388296724>'}]`}[\`${res}\`]${data.queue.current.isStream ? 'LIVE' : `\n${client.newDate(data.position)}/${data.queue.current.isStream ? 'LIVE' : client.newDate(data.queue.current.duration)}`}`)
