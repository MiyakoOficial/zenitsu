...
client.on('roleUpdate', async (oldRole, newRole) => {
    await LogsModel.findOne({ id: newRole.guild.id }, async (err, data) => {
        ...
        let embed = new Discord.MessageEmbed()
            ...
            .setColor(color)
        //console.log(await oldRole.guild.fetchAuditLogs())
        console.log(oldRole.guild.fetchAuditLogs().then(logs => logs.entries.first().changes.then(algo => { console.log(algo) })))
